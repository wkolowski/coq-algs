...
This definition of \m{QSArgs} differs from the previous one in that \m{A\ :\ Type} is now a field, whereas previously it was a parameter. In such cases we will call the class that has \m{A} as a field ``bundled'' and the one that has \m{A} as a parameter ``unbundled''.
