...
We can summarize strong normalization and confluence in one sentence by saying that ``every term has a unique normal form''. These normal forms can be described explicitly: they are generated by introduction rules\footnote{And also formation rules, when we're talking about types.} and elimination rules applied to variables. For example, some terms of type $\N$ in normal form are $0$ (zero), $\suc{\suc{n}}$ (successor of the successor of a variable), $\recN{X}{z}{s}{n}$ (recursor applied to a variable, provided that $X$, $z$ and $s$ are in normal form), etc.

The situation gets even simpler in the empty context: there are no variables in the empty context, so normal forms can't be variables, and because there are no variables, normal forms can't be elimination rules applied to variables either. So, in the empty context all normal forms start with an introduction rule. The metatheoretic property that expresses this is called ``progress'': in the empty context, every term $t$ either starts with an introduction rule or there is another term $t'$ to which $t$ computes.

But how do we know which normal forms correspond to which types? This may seem obvious, but we actually need two other very important properties related to types to establish this. The first one is uniqueness of types: if we have $\term{\Gamma}{t}{A}$ and $\term{\Gamma}{t}{A'}$, then we can derive $\typeconv{\Gamma}{A}{A'}$. Intuitively: the type of every term is unique. The second property is called either  ``(type) preservation'' or ``subject reduction'', and says that if $\term{\Gamma}{t}{A}$ and $t \leadsto t'$, then we can derive $\term{\Gamma}{t'}{A}$. This one is also easy to grasp intuitively -- it means that the computation of a term preserves its type.

The two above properties basically establish that the normal forms of $\N$ are generated by $0$, $\m{suc}$ and (in a non-empty context) $\m{rec}_\N$ and variables. Note that even though normal forms in the empty context start with an introduction rule, they aren't made only of introduction rules. The culprit here is the function type, whose normal forms can look like $\lambda n. \recN{X}{z}{s}{n}$ (provided that $z$ and $s$ are in normal form) -- it starts with an introduction rule, but then follows an elimination rule applied to a variable bound by the lambda. We thus define a base type to be basically any type besides the (dependent) function type. A canonical form is a term generated solely by introduction rules. This allows us to state the second most important metatheoretic property of type theory, called canonicity: in the empty context, a term of a base type computes to canonical form.

An immediate corollary of canonicity is consistency of type theory as a logic, i.e. in the empty context we can't construct a term of the empty type. Let's assume to the contrary that type theory is inconsistent, i.e. that we have some $\term{}{e}{\Empty}$. But then, by canonicitiy, $e$ computes to a canonical form, which means that $e$ starts with one of the introduction rules for type $\Empty$. But there are no such rules for $\Empty$! Therefore, we have a contradiction and type theory is consistent.
