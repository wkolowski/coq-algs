...
\date           {Wrzesień 2021} % Data zlozenia pracy
...
Uniqueness rules, on the other hand, describe what happens when we first use an elimination rule and then an introduction rule. For example, the uniqueness rule for products states that given $\term{\Gamma}{x}{A \times B}$ we can derive $\termconv{\Gamma}{(\pi_1(x), \pi_2(x))}{x}{A \times B}$. When read from left to right, this rule says that if we take $x$ apart using projections (the elimination rules) and then put them together into a pair (the introduction rule), we get the same $x$ that we started with. Alternatively, when read from right to left, this rule says that every term $x$ of type $A \times B$ is a pair of terms of type $A$ and $B$ (which happen to be the projections of $x$). In short: terms of the product type are pairs.

There is also another way of stating the uniqueness rules, whose flavour is less computational and more equational. It is very rare and barely ever appears in the literature. In this presentation, uniqueness rules tell us when two terms of a given type are convertible. For example, the uniqueness rule for products stated in this way says that given $\termconv{\Gamma}{\pi_1(x)}{\pi_1(y)}{A}$ and $\termconv{\Gamma}{\pi_2(x)}{\pi_2(y)}{B}$, we can derive $\termconv{\Gamma}{x}{y}{A \times B}$. In words, this means that terms of the product type are convertible when their first and second projections are convertible.\footnote{Note that this rule is different from the similar-looking congruence rule which says that given $\termconv{\Gamma}{a_1}{a_2}{A}$ and $\termconv{\Gamma}{b_1}{b_2}{B}$ we can derive $\termconv{\Gamma}{(a_1, b_1)}{(a_2, b_2)}{A \times B}$. This congruence rule only applies to pairs, not to arbitrary terms of type $A \times B$. The point of the uniqueness rule for products is precisely that it ensures that all terms of the product type are pairs.}

Uniqueness rules are completely optional -- a type need not have any. For example, almost all presentations of type theory lack a uniqueness rule for the natural numbers. In general, uniqueness rules are common for the unit type, the product type, the function type and other negative types, whereas they are very rare for positive types.\footnote{The reason for not including uniqueness rules for positive types is mostly that they are hard to implement, but there also are some model-related concerns. See \cite{Eta} for how to formulate a uniqueness rule for the natural numbers and for a more philosophical discussion of uniqueness rules in general. Also see \cite{Polarity} for a lightweight blogpost introducing the distinction between positive and negative types.}
