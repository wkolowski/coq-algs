...
% Another kind of further work: describe how to create a suite of default typeclasses that take even more work off the user's shoulders.

\paragraph{Related and further work}{The approach presented in this thesis is powerful, but not complete. Although we have seen some criteria for judging specifications and some techniques for improving them, it would be nice to have a general method for coming up with good specifications. Moreover, we have skipped the issue of actually inventing the algorithm that we want to formalize. The classical-imperative paradigm knows many techniques of algorithm design and most of them transfer easily to the functional paradigm, but it would be very interesting to see how they interact with the rest of our approach -- maybe it's possible to synthesise the algorithm directly from the specification, or maybe the abstract template plays nicer with greedy algorithms than with dynamic programming?

It would also be worth to explore how the other algorithmic activities described in section \ref{worlds}, like complexity analysis, benchmarking and testing, fit into our approach. An interesting link between complexity analysis and inductive types is Analytic Combinatorics \cite{AnalyticCombinatorics}, whose slogan is ``If you can specify it, you can analyze it''. There has also been some work on property-based testing in Coq \cite{SF4}. Although testing is almost never sufficient to prove the absence of bugs (except in very simple situations when there are finitely many possible inputs), it extremely efficient at generating counterexamples to our theorems. This can help us realize that our algorithm is incorrect many orders of magnitude faster than when we simply can't prove it correct (in such a case there's always a dilemma: either it's wrong or we are incompetent at proving).

in our formally-verified setting testing may appear is never sufficient 
...
I consider this thesis to be a work of pedagogy rather than one of research -- basically, it's a lengthy tutorial intended to be accessible to people with only rudimentary knowledge of Coq.



Both PFDS \cite{Okasaki} and VFA \cite{VFA} assume that the reader already knows about the standard
}

\paragraph{Exercise}{No tutorial or textbook is complete without some exercises, so I will provide one too in case the reader wants to practice whatever he learned after reading this thesis.
...
Design a sorting algorithm that generalizes both quicksort and mergesort. Give a specification of this algorithm, implement it and prove it correct.}
