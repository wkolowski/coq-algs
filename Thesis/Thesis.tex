...
A much more important tenet of our approach could be called \textit{hole-driven development}.\footnote{Sadly, there is no solid literature on the topic, mostly just blog posts that introduce the idea.} The idea here is that we construct programs in stages, by refining earlier, incomplete versions of the program. Each missing piece of the program is marked with an hole and refining an incomplete program is done by filling (i.e. replacing) the hole with a program of the appropriate type, which can also possibly be incomplete and contain further holes. This process is iterated until we reach a complete program.

The most well-known languages that support holes are Agda, Idris and Haskell \cite{TypedHoles}. In Idris, a hole looks like \m{?h :\ A}, where \m{h} is the name of the hole and \m{A} is its type -- holes are intrinsically typed, just like all other terms in the language. Coq, sadly, does not support holes. The closest equivalent of holes in Coq are \textit{existential variables} \cite{ExistentialVariables}. When we combine them with tactic-based definitions\footnote{These are programs defined between \m{Proof.} and \m{Defined.} using ordinary tactics.} using the tactic \m{refine}, we got something that vaguely resembles the hole-driven development available in Idris.

But wait! What we did when defining the quicksort template looked nothing like the hole-driven development approach that we just described. That's right, and for two separate reasons. First, we used holes quite implicitly, because quicksort is too easy an algorithm to pose serious problems when defining it. Second and more importantly, we filled our implicit holes with subprograms that we abstracted over and put into \m{QSArgs}, effectively postponing the hole-filling until the time when a user actually wants to run the algorithm.

What we did when defining the terminating quicksort template also didn't look like hole-driven development as described above, but it was. Recall that in section \ref{wellfounded}, after having defined the domain predicate, we tried to prove by well-founded induction that all lists belong to the domain, but we failed. The reason of our failure was that some termination-related properties of our subprograms were lacking and thus our proof had some gaps.\footnote{Curiously, mathematicians tend to use the word ``gap'' for a missing part of a proof. This is very similar to our use of ``hole'' for a missing part of a program. But there's also a significant difference between gaps and holes: in the mathematical world, gaps arise from mistakes and errors, whereas holes in programs come into existence by being deliberately put there.} We then assumed that the desired properties indeed hold by defining \m{TerminatingQSArgs}, and filled the gaps with proofs coming from there.

We see that our approach, based on filling holes with things we abstract over, is very powerful and lends itself well to both writing programs and proving theorems. But we don't need to treat all holes this way -- we can still, of course, fill holes with concrete terms, like in the traditional hole-driven development. This is indeed how we will prove our quicksort correct: we will reason by functional induction and use the tactic \m{admit} for any parts of the theorems that we can't prove. Then we will step back and try to turn these \m{admit}ted gaps into lemmas. When it's clear we couldn't possibly prove some lemma, we will simply assume that it holds by adding it into \m{VerifiedQSArgs}, the last incarnation of \m{QSArgs} which allows the correctness proof to go through.

Before we finish our deliberations, we should also mention the idea of \textit{type-driven development} \cite{TypeDriven}, sometimes also called \textit{type-first development} \cite{TypeFirst} or \textit{typeful programming} \cite{Typeful}.
