...
It is important to reiterate that not all judgements are derivable. For example, the judgement $\term{}{5}{\N \to \N}$ asserts that five is a function from natural numbers to natural numbers, but this is clearly nonsense. To know which judgements are derivable (and which are not), we need inference rules. An inference rule in general has the form ``if we can derive the judgements $J_1$, \dots, $J_n$, then we can derive the judgement $J$''. In most texts, inference rules are written with a horizontal line notation, with premises above the line and the conclusion below it, but we will state them inline, using ordinary sentences. An important special case are inference rules with no premises, i.e. of the form ``the judgement $J$ is derivable''. Such a rule declares that the judgement $J$ holds unconditionally. For rules with no premises, we will identify the whole rule with the judgement $J$.

A particular flavor of type theory (in our case, MLTT) is defined by specifying the rules for deriving its judgements.

How exactly do the contexts work? First, there is the empty context, usually denoted by leaving empty space to the left of $\vdash$. For example, $\type{}{\N}$ asserts that $\N$ is a type in the empty context. Second, if we have $\Gamma \vdash A$, (i.e. $A$ is a type in the context $\Gamma$), then we can extend the context $\Gamma$ with a variable of type $A$, written $\Gamma, x : A$, provided that the name $x$ doesn't already occur in $\Gamma$.

The rules that govern type formation are called, unsurprisingly, formation rules. For example, if in a context $\Gamma$ we can form the types $A$ and $B$, written $\type{\Gamma}{A}$ and $\type{\Gamma}{B}$ respectively, then we can form the type of functions from $A$ to $B$, written $\type{\Gamma}{A \to B}$.
