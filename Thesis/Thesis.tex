...
As a model of computation, type theory is a bit unusual, because the most important things it is concerned with are not programs or computations -- as the name suggests, its basic objects of interest are types. We can think of types as specifications that prescribe what is to be computed. In order to be meaningful, a type must first be formed. For example, we can form the type $\N$ of natural numbers and the type $\N \to \N$ of functions from natural numbers to natural numbers. The rules that govern type formation are called, unsurprisingly, formation rules.

The formal counterpart of the informal notion of program (or algorithm) is called a term. Types are inhabited by terms, which we write as $x : A$ and read ``$x$ has type $A$'' or ``$x$ is of type $A$''. This corresponds to the informal notion of a program satisfying its specification. For example, the type of natural numbers $\N$ has as terms, among others, $4$ and $2 + 2$, written $4 : \N$ and $2 + 2 : \N$, whereas $\lambda n. n + 5 : \N \to \N$ is the function that adds $5$ to its argument.

The notions of type and term are not absolute. What types can be formed and what terms inhabit a type depends, in general, on the assumptions that we make. For example, the type $\Fin{n}$ that has exactly $n$ elements\footnote{We will explain what an ``element'' is in a few paragraphs.} can be formed only under the assumption that $n : \N$, which we formally write as $\type{n : \N}{\Fin{n}}$. Another example: we have $n + m : \N$ only under the assumptions that $n : \N$ and $m : \N$, which we can write as $\term{n : \N, m : \N}{n + m}{\N}$.

We keep track of these assumptions using contexts. How exactly do the contexts work? First, there is the empty context, usually denoted by leaving empty space to the left of $\vdash$. For example $\type{}{\N}$ means that we can form the type $\N$ in the empty context. Second, if we have $\Gamma \vdash A$, (i.e. in context $\Gamma$ we can form the type $A$), then we can extend the context $\Gamma$ with a variable of type $A$, written $\Gamma, x : A$, provided that the name $x$ doesn't already occur in $\Gamma$.



For example, if in a context $\Gamma$ we can form the types $A$ and $B$, written $\type{\Gamma}{A}$ and $\type{\Gamma}{B}$ respectively, then we can form the type of functions from $A$ to $B$, written $\type{\Gamma}{A \to B}$. 

From now on we won't emphasize context-dependence, as everything we do in type theory depends on context.

Rules for creating terms can be divided into two main genres: introduction rules and elimination rules. Introduction rules for a type tell us how to create new terms of this type. For example, the introduction rules for $\N$ are $\term{\Gamma}{0}{\N}$, which says that $0$ is a natural number, and $\term{\Gamma, n : \N}{\suc{n}}{\N}$, which says that the successor of $n$ is a natural number given that $n$ is a natural number.
