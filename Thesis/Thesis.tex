...
The remaining part of the proof is also much simpler than in section \ref{wellfounded}. When using \m{Function} we don't need anymore to manually perform \m{destruct}s, apply the induction hypothesis or decide which constructor of the domain predicate to use when. The only proofs that are left to us are those establishing that the length of the argument descreases in the recursive calls.

To sum up, the current termination proof is only 5 lines long, whereas the previous one was 12 lines -- we got rid of almost 60\% of the work, all of which was just bookkeeping. But we shouldn't be too happy: automation won't dispel all our problems with function definition and termination proofs, because the \m{Function} command can't handle some kinds of functions.
...
The offending classes of recursive functions are the same ones that give us trouble with manual definition using the inductive domain method, namely those that exhibit \textit{nested recursion} and \textit{higher-order recursion}. Two examples of such functions are shown in the listing above.
...
The second function, called \m{tmap}, is way of mapping its argument \m{g} over every node of a non-empty tree with finite branching. This function exhibits higher-order recursion because the expression \m{map (tmap g) l}: the recursive call to \m{tmap} is partial, i.e. not fully applied, and in this form it is passed to \m{map} as argument. This scheme of recursion is less problematic than nested recursion and therefore this time the \m{Function} command doesn't refuse to cooperate. Moreover, we don't even need to argue about termination of this function -- it is accepted after passing the ordinary guardedness check, which means that Coq considers it structurally recursive. But there's still a bit of a problem -- after the function is defined, we get two warning messages telling us that \m{Function} couldn't generate definition of the graph of the function and that it couldn't generate the functional inversion principle.
