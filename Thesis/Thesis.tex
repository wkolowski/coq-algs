...
Types and terms are manipulated using judgements. The two basic kinds of judgements are the type formation judgement $\type{}{A}$, which asserts that $A$ is a type, and the typing judgement $\term{}{t}{A}$, which asserts that $t$ is a term of type $A$.\footnote{We can also read this as ``$t$ has type $A$'' or ``''$t$ inhabits type $A$''.} For example, the judgement $\type{}{\N}$ asserts that $\N$ is a type (as we will see, it is the type of natural numbers). Similarly, the judgement $\type{}{\N \to \N}$ asserts that $\N \to \N$ is a type (of functions from natural numbers to natural numbers). The judgement $\term{}{2 + 2}{\N}$ asserts that $2 + 2$ is a natural number, whereas $\term{}{\lambda n. n + 5}{\N \to \N$} asserts that the function which adds $5$ to its argument is a function from natural numbers to natural numbers.

The notions of type and term are not absolute. What types can be formed and what terms inhabit a type depends, in general, on the assumptions that we make. For example, the type $\Fin{n}$ that has exactly $n$ elements\footnote{We will explain what an ``element'' is in a few paragraphs.} can be formed only under the assumption that $n$ is a natural number, which we formally write as $\type{n : \N}{\Fin{n}}$. Another example: $n + m$ is a natural number only under the assumptions that $n$ and $m$ are natural numbers, which we write as $\term{n : \N, m : \N}{n + m}{\N}$.

We keep track of these assumptions using contexts. When a context $\Gamma$ appears to the left of $\vdash$ in a judgement, it means that this judgement holds under assumptions from $\Gamma$. How exactly do the contexts work? First, there is the empty context, usually denoted by leaving empty space to the left of $\vdash$. For example, $\type{}{\N}$ asserts that $\N$ is a type in the empty context. Second, if we have $\Gamma \vdash A$, (i.e. $A$ is a type in the context $\Gamma$), then we can extend the context $\Gamma$ with a variable of type $A$, written $\Gamma, x : A$, provided that the name $x$ doesn't already occur in $\Gamma$.

A particular flavor of type theory is defined by specifying the rules for deriving its judgements. A rule in general has the form ``given judgements $J_1$, \dots, $J_n$, we can derive the judgement $J$''. The rules that govern type formation are called, unsurprisingly, formation rules. For example, if in a context $\Gamma$ we can form the types $A$ and $B$, written $\type{\Gamma}{A}$ and $\type{\Gamma}{B}$ respectively, then we can form the type of functions from $A$ to $B$, written $\type{\Gamma}{A \to B}$.

Rules for creating terms can be divided into two main genres: introduction rules and elimination rules. Introduction rules for a type tell us how to create new terms of this type. For example, there are two introduction rules for natural numbers. The first says that $\term{\Gamma}{0}{\N}$, i.e. that $0$ is a natural number. The second says that given $\term{\Gamma}{n}{\N}$, we have $\term{\Gamma}{\suc{n}}{\N}$, i.e. that when we have a natural number $n$, its successor $\suc{n}$ is also a natural number.
