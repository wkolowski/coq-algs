...
The above listing shows how the Inductive Domain Method looks in practice. We define an inductive family of predicates \m{QSDom}, one for each parameter \m{A\ :\ QSArgs}. \m{QSDom A l} can be read aloud as ``\m{l} belongs to the domain of quicksort'' and an element of \m{QSDom A l} represents the complete call graph of our previous implementation of \m{qs} when applied to \m{l}.
...
Given such a domain predicate, the implementation of the new version of \m{qs} is trivial. Both \m{A} and \m{l} become implicit arguments and the new principal argument is \m{d : QSDom A l}. We match on \m{d} and in the first case we sort the list \m{l} ad hoc, whereas in the second case we recursively sort the elements lesser and greater than the pivot and put them together with the pivot and the elements equal to the pivot. Note that we don't have to call \m{choosePivot} or \m{partition} -- we can retrieve all we need from the arguments of \m{Long}.
