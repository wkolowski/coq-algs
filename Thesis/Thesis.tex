...
\subsection{Foreseeing the UX}

Are we done with the template? Not quite -- how do we know that our template makes sense? There's the obvious criterion of conforming with the informal description of the algorithm, but that's only one part of the story.

Another part is user experience (often shortened to UX). It's not the first thing that comes to mind when thinking about either algorithms or anything formally verified, but nonetheless it's important to get it right. A quite obvious thing to do at this point is to enter the user's mind (and keyboard) and see what happens.
