...
The next hidden world, in which the complexity analysis takes its place, is the model of computation. In most cases it is not mentioned explicitly, just like the logic and set theory thing from the previous paragraph, but usually it's easy to guess it. Most algorithms are, after all, intended to be implemented on modern computers, and the model of computation most similar to the workings of real hardware is the RAM machine.

As we see, the typical solution of an algorithmic problem stems from a sequence (or rather, a loop) of activities which live in seven different worlds: the pseudocode, the implementation language, the world of informal math, the world of formal math, the model of computation and the hardware, and these six are all joined together by the algorithmist's mind.

Even though the above recipe and its variations work well in practice, as evidenced by the huge number of algorithms solving our everyday problems, an inquisitive person (perhaps a philosopher) could ask a plethora of questions about how all these worlds fit together:
