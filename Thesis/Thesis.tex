...
Are we done with the template? Not quite -- how do we know that our template makes sense? There's the obvious criterion of conforming with the informal description of the algorithm, but it's rather vague and thus fulfilling it is a rather weak guarantee of success -- it's too easy to sneak in errors or inconveniences that will later turn out to be problematic.
...
In typical algorithmic settings the answer is not very enlightening: the user is anybody who runs our algorithm and user experience means that the algorithm is fast (or at least fast enough for his purposes; correctness is taken for granted). In our case things are different, however: execution speed will never be satisfying\footnote{Coq users make various jokes along the lines of ``I don't run my programs anymore, I only prove them correct.''. However, it is possible to extract Coq programs into Haskell, OCaml or Scheme and get performance typical for these languages.} and correctness is even more taken for granted.

What matters to us is that the template is very abstract and, unless provided with some defaults, the user will have to fill it out in order to get a concrete algorithm running. Thence comes a better advice for ensuring quality of our abstract template.
...
Here's what happens when we try this advice for our quicksort template. We will want to sort some natural numbers using the most basic quicksort variant, and so we define: a list is short when it's empty; we can sort an empty list ad hoc by returning an empty list; and we can partition by filtering, just like in the Haskell version. But how do we choose the pivot? When the list has head and tail, we choose the head to be the pivot but in case the list is empty we run into

We want to make sure it sorts correctly by looking at the output of sorting a list of numbers. 

\subsection{Foretelling the proof}


The other part will be the proof of correctness and chances are huge that it won't go through for the first version of the template because of some design flaws. For inexperienced Coq users it will often require some back-and-forth between the template design phase and the proving phase to get things right, 

\begin{center}
    \textbf{Formally verified algorithm concept/technique \#2.5} \\
    Make sure that types of various components of your template won't obstruct the flow of information when proving correctness.
\end{center}
...
\section{An aside on user friendliness} \label{friendliness}
