...
We are mostly done describing type theory, but we will take some more time to list differences between our presentation and what may be found elsewhere. First, some presentations of type theory can also include other judgements. For example, there may be a context convertibility judgement -- because types like $\Fin{2 + 2}$ and $\Fin{3 + 1}$ can appear in assumptions, judging whether two contexts are the same may be of some importance. We didn't discuss substitution in our presentation of type theory, but there are typically two ways of doing it. The first one is to define a meta-level substitution function by recursion. The second one is to introduce even more judgements and inference rules: the explicit substitution judgement, the substitution convertibility judgement, and add a lot of inference rules for term and type convertibility which describe how the substitutions are actually performed. Yet others can drop the type formation judgement and type convertibility judgement and go with universes.
