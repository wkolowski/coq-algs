...
% TODO: membership predicate is propsition
...
{Coq}{Snippets/ImprovingPatchworkDefinitions.v}
...
Our final definition of sortedness is, in terms of usability, much better than the one we started with: it is a simple, three-clause inductive definition. It's easy to prove theorems about sorted lists by induction on the proof of sortedness. It's also much easier to prove that a list is sorted, because we no longer have to deal with indices, proofs of index inequality, equations, auxiliary functions defined by recursion and so on. All of these things are now baked into the definition and thus all the parts fit together perfectly.
...
{Coq}{Snippets/JustRight.v}

After all the praise inductive definitions got up to now, we might be tempted to declare victory and go on to implement the algorithm already, but that would be a bit hasty: this definition fails the ``easy to use'' criterion. This is even harder to see in advance than the clumsiness of our initial definition of sortedness -- most people would probably discover it as an empirical fact as soon as they tried to prove correctness of any nontrivial sorting algorithm. The reason is that some of them move list elements around in very different ways than allowed by the definition of \m{Permutation}. This is yet another reminder that finding good specifications is more an art than a science.
...
\section{User experience: concerete algorithm as a sanity check} \label{sanity1}
...
The first one, called \m{f}, is a very weird way of defining the identity function on natural numbers (if this function were accepted, an ordinary induction on \m{n} would suffice to prove that it is indeed the identity). This function exhibits nested recursion because of the expression \m{f (f n')}: first we call \m{f} with the argument \m{n'}, which is a structurally recursive call on a subterm of \m{n}, so everything is fine so far, but then we call \m{f} with the argument \m{f n'}, i.e. the result of the previous call, which is not a structurally recursive call. We annotate the function with \m{measure id n}, in order to argue about its termination using well-founded induction (when we measure a natural number with the identity function, the resulting well-founded relation is simply $<$). But this maneuver didn't help and we fail miserably -- because of nested recursion, the \m{Function} command literally refuses to even consider our function and gives us an error message.
...
The second function, called \m{tmap}, is way of mapping its argument \m{g} over every node of a non-empty tree with finite branching. This function exhibits higher-order recursion because of the expression \m{map (tmap g) l}: the recursive call to \m{tmap} is partial, i.e. not fully applied, and in this form it is passed to \m{map} as argument. This scheme of recursion is less problematic than nested recursion and therefore this time the \m{Function} command doesn't refuse to cooperate. Moreover, we don't even need to argue about termination of the function -- it is accepted after passing the ordinary guardedness check, which means that Coq considers it structurally recursive. But there's still a huge problem -- after the function is defined, we get two warning messages telling us that \m{Function} couldn't generate definition of the graph relation and that it couldn't generate the functional inversion principle. And without the graph relation, of course, we don't get any automation at all.

These two recursion schemes seem really hopeless and we shouldn't expect \m{Function} to be of any use when dealing with them -- the most robust way would be to directly, manually apply the inductive domain method. But not all hope is lost! There exists another Coq plugin that can assists us with functions, called \m{Equations} \cite{Equations}. It's workings are vastly different from those of \m{Function} (including a very different, Haskell-inspired syntax), but the end effect is pretty similar: we define a function the way we want it to behave, we get some assistance with the termination proof, and then after the function is accepted all the nice reasoning principles are generated for us automatically. The strength of \m{Equations} is that it seems to handle nested recursion just fine -- in the case of the function \m{f}, we don't even need to argue about termination, because \m{Equations} sees through it immediately. On the other hand, when it comes to higher-order recursion, we are still fundamentally out of luck. However, since simple functions of this kind are considered structurally recursive by Coq, it may be a good idea to just simply use ordinary induction.
