...
At last we can formulate the functional induction principle and prove it. The principle looks similar to the induction principle \m{QSGraph\_ind} that Coq generated automatically for \m{QSGraph}. The differences are that we assume the induction hypothesis holds for \m{P lt (qs A lt)} and \m{P gt (qs A gt)} instead of \m{lt'} and \m{gt'} coming from derivations of \m{QSGraph A lt lt'} and \m{QSGraph A gt gt'}. Another difference is that there is no mention of \m{QSGraph} in the conclusion.

The proof is straightforward. We introduce assumptions to context and apply the induction principle of \m{QSGraph}. In the first case we use an assumption. In the second case



\subsection{Automating the boring stuff}
