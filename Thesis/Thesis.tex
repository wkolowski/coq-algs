...
We will start with the additional assumptions we need, collecting them into a new class called \m{VerifiedQSArgs}. The first field of the class is called \m{T} and has type \m{TerminatingQSArgs}. This is similar to how we defined \m{TerminatingQSArgs} as an extension of \m{QSArgs}. In fact, this is the customary way to simulate inheritance in Coq.
...
which is also a coercion, because we want to be able to use an instance of \m{VerifiedQSArgs} wherever an instance of \m{TerminatingQSArgs} is expected.
