...
\date           {\sout{Czerwiec '20 chyba że koronawirus} \\ \sout{Jednak raczej wrzesień} \\ \sout{Z września też nici, ale spoko} \\ \sout{Może w marcu '21 się uda.} \\ Jak będzie to będzie.} % Data zlozenia pracy
...
The finished correctness proofs are shown in the listing above. They are not terribly different from the initial versions. The only exceptions are that their are now stated using \m{VerifiedQSArgs}, which makes them shorter, and they no longer contain any gaps. Also, the structure of the second proof changed a bit to reflect the fact that only two lemmas are needed now (as opposed to three \m{cut}s previously).

\section{Sanity check: default correctness proofs} \label{sanity3}

We are almost done with our formally correct quicksort. The last step that separates us from proclaiming total success is yet another sanity check -- we will have to provide concrete correctness proofs for the default implementation from section \ref{sanity2}.
...
We can then define the order relation with respect to which the output will be sorted as \m{R x y := p x y = true} -- we are esentially turning our comparison function into a relation, because we don't have anything else. Next, we need to provide a proof that our relation is reflexive, but functions of type \m{A -> A -> bool} in general don't need to have this property, so we need to assume it. Last but not least, it only makes sense to sort using a comparison function which guarantees that the order is total, \m{p x y = true \/ p y x = true}, which we express in a more convenient way as \m{p x y = false -> p y x = true}. Interestingly, we didn't need to assume totality during our proof by admission, but this time we will need it in one of the proofs.
...
But before we do that, we will take our time to put all our sanity checks into a broader perspective. We will be especially interested in how they are related to hole-driven development and proof by admission as described in the \hyperref[byadmission]{previous section}.
...
Type-driven development\footnote{Type-driven development is often shortened to TDD, but this acronym can also be expanded as \textit{test-driven development}, an Agile technique popular among users of dynamically typed languages. In this thesis, TDD will always refer to type-driven development, however.} is championed by Idris and described in the book \cite{TypeDriven} by the language's creator, Edwin Brady. There it is realised by a kind of interaction between the programmer and the language's tooling, where the programmer talks to the system by writing types and programs fragments, and the system responds by synthesizing obvious programs that fit the given type (which is reminiscent of Coq's proof automation), refining the program (e.g. performing case splits) or providing the programmer with suggestions when there are more possible implementations, and typechecking the program fragments provided by the user.

In Coq we can also use this approach as described above, at least as long as we stay in the proof mode. Outside the proof mode, support for type-driven development seems missing, but it's still possible to practice it, even though without tooling support. What we did in section \ref{sanity1}, when we defined the default concrete implementation of our algorithm, was pretty similar in nature to type-driven development. The most noticeable difference is that in Idris, type-driven development is usually practiced using precise, dependently typed specifications, whereas our quicksort template consists only of simply typed components.

However, we (and the users of our algorithm template) also have additional guidance from the termination properties in \m{TerminatingQSArgs} and correctness properties in \m{VerifiedQSArgs}. If we consider them all at once, instead of defining the concrete implementation in stages, what we get is precisely just an exercise in type-driven development -- as soon as we define anything of the appropriate type (i.e. \m{VerifiedQSArgs}), we are done. As it turns out, hole-driven development, together with our idea of abstracting some holes away, naturally leads to programs that can be proven correct using proof by admission, and then finally we can recover concrete, fully correct implementations using type-driven development. These three techniques are intrinsically linked, yielding a perfect harmony.

\concept{To ensure that your formally verified algorithm template provides the best user experience, use type-driven development to define and prove correct another (preferably quite different) concrete implementation.}
