...
Before we see how to use this new concept to improve our original template, we first have to meet its relatives: \textit{boolean blindness}, a dangerous disease which often afflicts programmers trasitioning from imperative to functional, and \textit{small-scale reflection}\footnote{See \cite{SSR1} for an implementation of a proof language for Coq which is based on this methodology and \cite{SSR2} for its documentation. Also see \cite{MCB}, a book about a library of formalized mathematics done in Coq which uses the small scale reflection methodology.}, a general-purpose proof engineering methodology most famously applied to the first fully formal proof of the four colour theorem \cite{4CT1} \cite{4CT2}.
...
% New perspective: implicit vs explicit information. Boolean blindness is when our information is too implicit. Extrinsic typing in lamda calc is implicit type information, intrinsic typing is explicit type info. Reflection is replacing some explicit information with implicit information realized by computation. Strong specification types in Coq (like sumbool, sumor) allow enriching thing with more explicit information. The same difference is present in data structures, e.g. we can have BSTs with explicit proofs of BST property, or ordinary trees which will satisfy the BST property implicitly when constructed using an appropriate interface.
