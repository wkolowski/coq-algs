...
The above technique outlines how to derive the functional induction principle. The first step is to notice that there are two inductive families whose induction principles are almost what we're looking for: the inductive domain predicate and the inductive graph relation. We have already seen the former and should be adept at using it, but it turns our not to be what we are looking for -- we will see why in the next section. We are thus left with the latter.
...
For example, we can define the graph of the function \m{div2}\footnote{\m{div2} divides a natural number by two, rounding down if the argument is odd.} in two extensionally equivalent ways, either by saying that \m{R a b := a = 2 * b $\lor$ a = 1 + 2 * b} or with an inductive definition that closely follows the recursive definition of \m{div2}. Only the latter, inductively defined graph relation is useful for deriving the functional induction principle. 
...
The next step is to prove that \m{QSGraph} is ``correct'' with respect to \m{qs}. In less jargony words, this corresponds to one of the implications from the definition of a function's graph, namely \m{f a = b -> R a b}. The statement of the lemma is a bit different, however: it is the equivalent of saying \m{R a (f a)} instead.

The proof is easy: we introduce assumptions to context, unfold the definition of \m{qs} and generalize the termination proof \m{QSDom\_all A l} to a general \m{q\ :\ QSDom A l}. The rest is a simple induction on \m{q}, followed by using the appropriate constructor and feeding it with the arguments it wants.
...
% TODO
To finish, \footnote{It was first described in an early form by Burstall in \cite{Burstall} and in the more modern form by \cite{Slind} and then reinvented many times \cite{Function} \cite{Program} \cite{FromTheLeft}, which shows that the technique is quite intuitive and natural.}
...
The bottom line is that in Coq without assuming additional axioms, we can't prove that an arbitrary type \m{A} satisfies UIP\footnote{UIP stands for \textit{Uniqueness of Identity Proofs}. A type \m{A} satisfies UIP when for all \m{x y\ :\ A}, any two proofs \m{p q\ :\ x = y} are equal.} and therefore we have no chance to prove that any two proofs of \m{short l = Some (h, t)} are equal. Assuming that all types satisfy UIP is consistent with Coq, but an axiom-dependent way of deriving functional induction principles is clearly inferior to an axiom-free one. Similarly, the variant of inductive domain method which uses \m{Prop} as the codomain also isn't viable, as it would require us to assume Proof Irrelevance.

The ultimate moral of this section's story is that even though the approach to deriving functional induction principles based on inductive graph relations presented in the last section is quite boilerplate-heavy, there is no way around it...

Or is there? Another idea that may come to a smart prover's mind is just to directly use the induction principle of the domain predicate in every single proof, either by first proving a lemma for \m{qs'}, or by unfolding the definition of \m{qs} and generalizing the termination proof in-place.

That would be fine if we wanted to just get a single small proof done quickly, but in a longer perspective it is not a good solution either. The crucial difference is that in case of the functional induction principle we rely on an abstract specification, whereas using induction on \m{QSDom} means relying on a concrete implementation. 
