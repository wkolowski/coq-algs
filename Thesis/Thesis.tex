...
Before we see how to use this new concept to improve our original template, we first have to meet its relatives: \textit{boolean blindness}\footnote{This is an informal term used in the functional programming blogosphere. It was coined most likely by Dan Licata in his lecture notes for an introductory functional programming class \cite{BBLicata}. See \cite{BBHarper} for a philosophical presentation, \cite{BBHXA} for a critique of the former viewpoint and \cite{BBFairbank} for a more software-engineering-oriented discussion.}, a dangerous disease which can afflict even the best functional programmers, and \textit{small-scale reflection}\footnote{See \cite{SSR1} for an implementation of a proof language for Coq which is based on this methodology and \cite{SSR2} for its documentation. Also see \cite{MCB}, a book about a library of formalized mathematics done in Coq which uses the small scale reflection methodology.}, a general-purpose proof engineering methodology most famously applied to the first fully formal proof of the four colour theorem \cite{4CT1} \cite{4CT2}.

Boolean blindness is a term s


% 1. Names: isPremium = true
% 2. Errors: unSome False None : False
% 3. Bad: if isZero n then ... else ...
