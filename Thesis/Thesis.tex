...
The focal ideas of \textit{generating}, \textit{counting} and \textit{permuting} yield definitions which are very different but equivalent.\footnote{As far as types with decidable equality are concerned.} Each definition satisfies our criteria of being easy to use and abstract enough, but they still vary greatly in their strengths and weaknesses and each is fit for different purposes. Or rather, they would vary and be fit fot different purposes if we were starting from scratch and had to prove all necessary lemmas ourselves. But in case all the properties of permutations we will need are readily available, we don't need to wory about the differences, because these properties are all that we need.\footnote{Another way not to worry about how we define permutations is to abstract over this definition by taking ``permutation'' to mean any relation \m{P : forall A : Type, list A -> list A -> Prop} that satisfies the properties we will need it to satisfy. This is a glimpse of the concepts and techniques we will see in the chapters to come.} This is indeed the case, as the original definition we gave (the one based on generating) is part of Coq's standard library, which is replete with lemmas about permutations that we can use in our proofs. We will therefore stick with this definition.
