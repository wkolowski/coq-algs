...
Therefore an implementation of a general recursive function is usually weaved of two closely knit parts: the algorithmic part (the algorithm) and the logical part (the termination proof). These parts are inseparable: the algorithmic part is littered with proofs and the termination proof of course strongly depends on the algorithm's properties.

In the next section we will see a technique that allows us to deal with these two parts separately and a Coq language feature that can automate this technique, but at present, in accordance with concept/technique \#2, we will ignore the matter of termination. A nice way to do this, recently introduced into Coq, is the option \m{Unset Guard Checking}, which can temporarily turn the termination checker off.
...
% Hints about turning the termination checker off. 
