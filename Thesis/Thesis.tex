...
\subsection{Foreseeing user experience} \label{UX}
...
And so we proceed by... well, we don't proceed anymore. We are stuck because we have no evidence to back up our reasoning from the previous one and a half paragraphs. The reason is simple: there's nothing in \m{TerminatingQSArgs} guaranteeing that \m{short}, \m{choosePivot} and \m{partition} don't produce gibberish.
...
But the beauty of mathematics in general and of our approach to algorithms in particular is that we can just assume everything we need and call it a day! This is indeed why we haven't shown the definition of \m{TerminatingQSArgs} yet -- it is shown in the listing below.
...
Coming up with the proper definition of \m{TerminatingQSArgs} is straightforward: start with a class that has a single field which accomodates the original \m{QSArgs} and then successively add all the lemmas and properties needed to finish the termination proof.
...
\subsection{Sanity check -- concrete termination proof} \label{UX2}

In section \ref{UX} we learned that an important aspect of our algorithmic work is user experience, which we defined as what happens when a user tries to fill in the algorithm template that we provide. It might have happened, after all, that our template is so silly that it isn't possible to fill it in!

Our present situation is quite similar: our termination ``proof'' is not really a proof, but a combination of a bunch of unverified assumptions that we made when our first proof attempt failed. If we made a mistake in our initial reasoning, it could have lead us to making too strong or even contradictory\footnote{Our assumptions can't be, however, too weak -- if they were, the termination proof wouldn't have gone through.} assumptions, which would render our algorithm template useless.
...
\subsection{Foreseeing user experience 1 -- avoiding bloatware}

\subsection{Foreseeing user experience 1 -- provide a default implementation}
