...
Having given a thorough introduction, it should now be clear what we mean by ``formally verified functional algorithms in Coq''. In the remaining chapters of the thesis we describe the actual ``concepts and techniques'', using quicksort -- a well-known and well-studied sortign algorithm -- for the purposes of concrete illustration. The rest of the thesis is structured as follows:

\begin{itemize}
    \item In \hyperref[specify]{chapter 2} we look at techniques for verifying a specification's usefulness and for improving bad specifications (sadly, there seems to be no easy way of coming up with a good specification).
    \item In \hyperref[template]{chapter 3} we show how to implement an abstract template of an algorithm without worrying too much about concrete details or the termination proof. We also present techniques for assessing the quality of such a template.
    \item In \hyperref[termination]{chapter 4} we describe a general method for carrying out termination proofs and review techniques for reasoning about functions defined using this method.
    \item In \hyperref[exnihilo]{chapter 5} we show how to formally prove the abstract algorithm template correct in a top-down fashion without writing too many proofs.
\end{itemize}
...
Such an idea is \textit{counting}: two lists are permutations of one another when they contain the same number of occurrences of each \m{x\ :\ A}\footnote{The proof of equivalence of these two definitions of \m{Permutation} can be found in the thesis' repository. I won't give a precise location here, because it's likely to be out of date soon.} Note that, because we count elements, this new definition only applies to types that have decidable equality. This restriction is not a big problem in practice, because the need to sort functions, streams, or other such infinite objects lacking decidable equality, is pretty rare.
...
\section{A slight variation on a theme}
...
\section{Sanity check -- concrete proofs}

\concept{Prove the concrete algorithm correct by filling out the proofs.}

\section{User experience: avoid bloatware}
