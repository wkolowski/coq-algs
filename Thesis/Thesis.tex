...
But not all functions are as tame as quicksort -- there exist some nasty functions that could resist the approach presented above. An example that instantly comes to mind is McCarthy's f91 function\footnote{This is a function which satisfies the equation \m{f n = if 100 < n then n - 10 else f (f (n + 11))}.} and, more generally, any function with \textit{nested recursion}, i.e. in which the result of a recursive call is an argument of another recursive call.
