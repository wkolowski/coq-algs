...
As we said, the codomain of \m{QSDom} changes to \m{Prop}, but everything else stays the same.
...
Next we need an inversion lemma that retrieves a proof of \m{QSDom A lt} from a proof of \m{QSDom A l}. It realizes in practice the exception to the rule we mentioned before. Even though when matching on a proof of \m{QSDom} there are two possible cases, \m{Short} and \m{Long}, we will match it only when we know the list is not short, which reduces the number of possible cases to one.

The proof is a bit involved because of all the equational premises which need to be rewritten at some point, but the idea is very simple: just retrieve a proof of \m{QSDom A lt} from the constructor's arguments. We also need an analogous inversion lemma for \m{QSDom A gt} -- it is not shown in the listing because it is completely analogous.
