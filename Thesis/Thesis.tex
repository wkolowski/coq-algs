...
The definition we get as a result is not abstract enough, because not all types support decidable equality, so the the last step is to generalize from counting occurrences of elements to counting occurrences any elements satisfying a general decidable property. This is what was meant by ``changing the focus from one aspect to another''. This time a more patchwork-y definition turned out to be better.

The final specification of a sorting function that we have arrived at through our considerations looks as follows.
