...
\begin{itemize}
    ...
    \item Split the list/array (with pivot removed) into two lists/arrays, one of which contains elements less than the pivot, while the other contains elements greater than or equal to the pivot.
    \item Sort both lists/array recursively and put them together with pivot in the middle.
\end{itemize}
...
If you have ever seen the famous two-line implementation of quicksort in Haskell (shown in listing \ref{HaskellQuicksort}), you know that it may be very easy to implement at least the list version of quicksort. If you have ever seen an implementation in an imperative language (a typical implementation in C is shown in listing \ref{CQuicksort}), you might be starting to feel uneasy. Our formally correct implementation will look much scarier and be much longer than the typical imperative implementation, but in the end it will be just as simple and beautiful as the Haskell implementation.
