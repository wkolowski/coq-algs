...
(* Alternative version with nested recursion. *)
(*
Fixpoint size {A : LinDec} (h : PairingHeap A) : nat :=
match h with
    | E => 0
    | T _ l =>
        let sizes :=
          fix sizes (l : list (PairingHeap A)) : nat :=
          match l with
              | [] => 0
...
Lemma size_deleteMin :
  forall (A : LinDec) (h : PairingHeap A),
    h <> E -> size (deleteMin h) = pred (size h).
Proof.
  induction h using Tree_ind_proper; intros.
    congruence.
    cbn. apply size_mergePairs.
Qed.

Function toList {A : LinDec} (h : PairingHeap A) {measure size h}
  : list A :=
match h with
    | E => []
    | T m l => m :: toList (deleteMin h)
end.
Proof.
  induction h using Tree_ind_proper; intros; inv teq.
  rewrite size_deleteMin.
    cbn. apply le_n.
    congruence.
Defined.
...
Fixpoint pairingSort (A : LinDec) (l : list A) : list A :=
  toList (fromList l).